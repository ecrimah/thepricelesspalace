...
export async function GET(req: NextRequest) {
    ...
    try {
        if (action === 'stats') {
            ...
            const [assignmentsRes, ridersRes, todayAssignments, pendingOrders, zonesRes] = await Promise.all([
                ...
                supabaseAdmin.from('orders')
                    ...
                    .not('id', 'in', `(${(await supabaseAdmin.from('delivery_assignments').select('order_id').not('status', 'in', '("failed","returned")')).data?.map((a: any) => a.order_id).join(',') || '00000000-0000-0000-0000-000000000000'})`),
            ...
            const stats = {
                ...
                activeDeliveries: allAssignments.filter((a: any) => ['assigned', 'picked_up', 'in_transit'].includes(a.status)).length,
                deliveredToday: todayData.filter((a: any) => a.status === 'delivered').length,
                failedToday: todayData.filter((a: any) => a.status === 'failed').length,
                totalRiders: allRiders.length,
                activeRiders: allRiders.filter((r: any) => r.status === 'active').length,
                onDeliveryRiders: allRiders.filter((r: any) => r.status === 'on_delivery').length,
                pendingOrders: pendingOrders.count || 0,
                activeZones: zonesRes.count || 0,
                todayRevenue: todayData.filter((a: any) => a.status === 'delivered').reduce((sum: any, a: any) => sum + (a.delivery_fee || 0), 0),
            };
        ...
        if (action === 'unassigned') {
            ...
            const excludeIds = (assignedOrderIds || []).map((a: any) => a.order_id);
