...
| **House of Elle** (`houseofelle`) | Jul 2026 | Shape A — staging promoted to prod; UUID `id` defaults restore fix |
| **Four prod stores** (raymahomes, vees, badawiasimports, grocerydistrict) | Jul 2026 | Shape A hardening playbook pass |
| **Eight staging stores** (amaburland, houseofelle*, baddiescurves, fitaura, luxurystrand, rme, sohergh, wholesaleaccra) | Jul 2026 | Shape A hardening on `staging/plain-postgres` (*houseofelle now prod) |
| **Kiyas Essentials** (`kiyas-essentials` → folder `kiyas`) | Jul 2026 | Prod `kiyas-app` — **no `@supabase/*`**, HTTP + PG shims; sslip until custom DNS |
| **Bold Beautiful** (`boldbeautiful`) | Jul 2026 | Prod `boldbeautiful-app` — **no `@supabase/*`**, HTTP + PG shims; sslip until custom DNS |
...
**Do not clone another store's Coolify env.** When creating an app from a template (copy build settings only), set **per-store** secrets from that project's Vercel export (`vercel-env-export/<repo>/.env.production`) or owner-provided `.env` — JWT, `RESEND_API_KEY`, Moolre/Paystack, reCAPTCHA, `STORAGE_SIGNING_SECRET`, Supabase anon/service keys (for shim compat), etc. Only share the **cutover trio** pattern (`DATABASE_URL`, `NEXT_PUBLIC_USE_PLAIN_PG`, app-origin `NEXT_PUBLIC_SUPABASE_URL` / `NEXT_PUBLIC_APP_URL` / `STORAGE_PUBLIC_URL`). Use `scripts/apply-coolify-env-from-file.sh`.

**Fleet cannot create brand-new Coolify apps** — only `apps` / `app` / `deploy` / `db list` / `db provision`. New apps need Coolify owner UI, or promote an existing staging app (see §1b).

**Deploy one app at a time.** Parallel Coolify/nixpacks builds often collide and fail; queue a single `sudo fleet deploy`, wait until the live image SHA matches, then start the next.
...
### 1a. Restored DB — UUID `id` defaults (critical for checkout)

After Supabase dump → plain Postgres restore, **`id uuid PRIMARY KEY` columns often lose their `DEFAULT`**. Checkout inserts (Shape A) typically omit `id` and expect the DB to generate it.

**Symptom (browser alert):**

```text
Failed to place order: null value in column "id" of relation "orders" violates not-null constraint
```

**Detect** (from the app container or as DB owner):
...
Also confirm `pgcrypto` (or equivalent) is installed so `gen_random_uuid()` works.

**Fix once per database** (run as Postgres **owner/superuser** — the Coolify app DB role usually cannot `ALTER TABLE`):
...
Typical tables that need it: `orders`, `order_items`, `customers`, `profiles`, `cart_items`, `wishlist_items`, `addresses`, `notifications`, `audit_logs`, returns/support/review tables, etc.

**Commit a matching migration** under `supabase/migrations/` (or `scripts/`) so staging/prod stay aligned. House of Elle example: `20260726170000_uuid_id_defaults.sql`.

**Run this check before the first real checkout** on every newly restored store DB (not only after a customer reports it).

### 1b. Promote staging → production (when no `*-app` exists)

`fleet` cannot create Coolify apps. When DNS already points at the VPS and only `*-staging` exists:

1. Point DNS `@` + `www` A records at **`169.58.8.203`** (or current big-vps IP).
2. In Coolify (UI or artisan/tinker): rename app → `*-app`, set FQDN to `https://example.com,https://www.example.com`.
3. Update **both** runtime and preview env rows for:
   - `NEXT_PUBLIC_APP_URL`
   - `NEXT_PUBLIC_SUPABASE_URL`
   - `STORAGE_PUBLIC_URL` (if used)
   to the **production origin**.
4. **Regenerate Traefik `custom_labels`** from the new FQDN (`generateLabelsApplication` → base64 into `applications.custom_labels`). Editing `fqdn` alone is not enough — stale labels keep routing to `*-staging.*.sslip.io` → **503 / no available server** on the custom domain.
5. `sudo fleet deploy <app-name>`; confirm image SHA + Traefik `Host(\`example.com\`)` labels on the running container.
6. Smoke home/shop/API + service worker; then delete the staging Coolify resource only if promotion reused that app (or delete the old staging app if you created a separate prod app).

House of Elle (Jul 2026): staging UUID `xeal036txnby77t6oycflmol` promoted in place to `houseofelle-app` → https://houseofelle.store .
...
| Parallel deploys fail / queue stuck | Deploy **one** app; wait for finished + live SHA before the next |
| Custom domain **503** / self-signed / “no available server” after FQDN change | Stale Traefik `custom_labels` still on sslip host — regenerate labels + redeploy (§1b) |
| Checkout: `null value in column "id" of relation "orders"` | Missing UUID defaults after restore — §1a |
| Palette / CSS “didn’t change” on production | Almost always stale container SHA, not Tailwind |
| SW 404 on `/service-worker.js` (some src-layout apps) | Ensure `public/service-worker.js` ships, or rewrite → `/api/service-worker` |
...
- [ ] **DB:** every public UUID `id` has `DEFAULT gen_random_uuid()` (§1a) — place a test order
- [ ] Prod FQDN Traefik rules match custom domain (not leftover sslip) if promoted (§1b)
...
2. After any dump/restore: apply UUID `id` defaults (§1a) **before** checkout QA.
3. Verify REST/auth/storage **or** native `/api/*` + uploads mount.
4. Fix build blockers; set `images.unoptimized` + writable `.next/cache` on Dockerfile runners.
5. Fix service worker (§16) before heavy PWA testing.
6. `lib/format-money` + admin/storefront `.toFixed` audit (§10a).
7. Wire newsletter + merchant emails.
8. Admin customers live aggregation.
9. Product SEO helper + backfill.
10. Blog real editor or hide module.
11. Address book API + checkout autofill + order history actions.
12. Storefront UX / shipping / returns / footer / auth stub audit.
13. Full verification checklist → deploy **one app at a time** → **re-check image hash** + PWA refresh.
14. Production cutover: DNS → promote/create prod app (§1b) → smoke checkout → remove staging only when prod is healthy.
...
| Item | Value |
|------|--------|
| Repo | `katalambano878/houseofelle` |
| Coolify app | `houseofelle-app` only (**staging promoted in place** Jul 2026) |
| UUID prefix | `xeal036txnby77t6oycflmol` |
| Production | https://houseofelle.store |
| DB name (fleet) | `houseofelle_staging` (name kept after promote — fix env URLs, not necessarily rename DB) |
| Shape | A — shimmed Supabase-js → plain PG |
| Branch | `staging/plain-postgres` |
| Notable (Jul 2026) | Hardening `sw-v2.5-houseofelle` → DNS A→VPS → promote staging→prod + regenerate Traefik labels → **UUID `id` defaults** on orders/order_items/… after checkout null-id failure |

```bash
BASE=https://houseofelle.store
ssh big-vps "sudo docker ps --format '{{.Image}}' | grep xeal036"
curl -sk -o /dev/null -w "%{http_code}\n" "$BASE/"
curl -sk "$BASE/service-worker.js" | head -n 3
# As postgres: SELECT column_default FROM information_schema.columns
#   WHERE table_name='orders' AND column_name='id';  -- expect gen_random_uuid()
```

### Recent Shape A hardening batch (Jul 2026)

| Coolify | Status note |
|---------|-------------|
| `raymahomes-app`, `vees-app`, `badawiasimports-app`, `grocerydistrict-app` | Prod hardened + SW `sw-v2.5-*` |
| `amaburland-staging`, `baddiescurves-staging`, `fitaura-staging`, `luxurystrand-staging`, `sohergh-staging`, `wholesaleaccra-staging` | Staging hardened; prod cutover pending |
| `rme-staging` | Hardening in git; live cutover deferred (SW/path layout edge case) — do not delete staging until fixed |

Local clones live under `C:\Users\hp\OneDrive\Desktop\websites\<store>` on branch `staging/plain-postgres`.
