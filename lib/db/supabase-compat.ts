...
class QueryBuilder implements PromiseLike<{ data: any; error: any; count: number | null }> {
  ...
  private async resolveEmbeds(
    rows: Row[],
    parsed: ParsedSelect,
    /** Table the current `rows` belong to (must update on nested embeds). */
    ...
    for (const embed of parsed.embeds) {
      ...
      // from the *parent* table to the embed table means forward (object);
      ...
      if (fk) {
        ...
        // Nested embeds are relative to the related table, not the root query table
  ...
  private findReverseEdge(relatedTable: string, parentTable: string): FkEdge | undefined {
  ...
  return {
    ...
    admin: {
      async createUser(attrs: {
        ...
        if (error || !user) {
          return { data: { user: null }, error: { message: error || "Failed to create user" } };
        }
        return { data: { user }, error: null };
      },
      async listUsers(opts?: { page?: number; perPage?: number }) {
        ...
        try {
          ...
          for (const row of res.rows) {
            const u = await auth.getUserById(row.id);
            if (u) users.push(u);
          }
          return { data: { users }, error: null };
        } catch (e: any) {
          return { data: { users: [] }, error: { message: e.message || String(e) } };
        }
      },
    },
  };
